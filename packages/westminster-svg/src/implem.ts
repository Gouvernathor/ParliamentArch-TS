...
function populateHeader(
    ...
    svg.setAttribute("viewBox", `${
        ...
        -Math.max(speak.nRows/2, opposition.nRows && opposition.nRows+1, cross.nRows/2)
    } ${
        speak.nCols + Math.max(government.nCols, opposition.nCols) + (cross.nCols && cross.nCols+1)
    } ${
        Math.max(
            speak.nRows/2, opposition.nRows && opposition.nRows+1, cross.nRows/2,
        ) + Math.max(
            speak.nRows/2, government.nRows && government.nRows+1, cross.nRows/2,
        )
    }`);
